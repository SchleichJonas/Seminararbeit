...
def plotsSite():
    ...
    with st.expander("Select release dates to be included in the plot", expanded=True):
                selected_map = {}
                for entry in IMPORTANTDATES.keys():
                    selected_map[entry] = st.checkbox(f"{entry} - {IMPORTANTDATES.get(entry)}", value=True)
            ...
            try:
                ...
                if(st.session_state.query != ""):
                    ...
                    if df.empty:
                        ...
                        fig.update_layout(uirevision=str(selected_dates))
                        st.session_state.last_dates = selected_dates
                        for event_name, event_date in IMPORTANTDATES.items():
                            if(st.session_state.dates and selected_map[event_name]):
